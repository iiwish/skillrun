...
SkillRun Core
  - Rust runtime
  - reads Manifest
  - carries schemas and validates source hashes
...
- 在支持 metadata phase 的层级中报告 schema、capability 和 runtime requirements。
...
`output` 是规范字段。部分现有 language adapter wrapper 可以接受 action return value 中的 `result` 作为作者侧兼容输入，但 adapter 写给 Core 的标准 envelope 应使用 `output`。
...
- stdout text 不会成为 result 或 output。
