...
For normal installs, choose the installer for your OS first:

| User path | Release asset |
| --- | --- |
| macOS / Linux install or update | `skillrun-installer.sh` |
| Windows install or update | `skillrun-installer.ps1` |
| Manual/offline install, package managers, or CI pinning | The matching platform archive |
| Archive checksum verification | `sha256.sum` |

GitHub may also show autogenerated `Source code` downloads on the release page. Those are repository snapshots for developers, not ready-to-run SkillRun CLI binaries.
