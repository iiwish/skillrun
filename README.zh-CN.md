...
SkillRun v0.5.0 是当前 release candidate binary/crate 版本。最近完成的 mainline patch release 是 v0.4.3；v0.5.0 定义 language-agnostic Adapter Protocol，并用 Level 0 command adapter 证明这条边界。v0.5.2 integration line 增加 Consumer JSON Surface；v0.5.3 增加本地 capsule registry 与 switchboard state；v0.5.4 在独立 Desktop 项目消费 Core 前硬化运行时和 JSON contract。
...
当前本地 binary 输出 `skillrun 0.5.0`；当前生成的 Manifest IR 与 IPC protocol version 仍是 `0.1.0`。v0.5.4 硬化 Core 行为和 JSON contract，但不自动改变这些协议版本。
