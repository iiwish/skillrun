...
- 当前实现：`T001` Rust CLI skeleton。
- 当前可用：`skillrun --help`、`skillrun --version`，以及 CLI skeleton 的 contract tests。
- CLI 已列出计划中的 MVP 命令，但 `init`、`manifest`、`inspect`、`test`、`run`、`serve` 和 `pack` 尚未实现。
...
计划命令在实现前会明确返回 `command not implemented yet`，避免让用户误以为 runtime 行为已经落地。
