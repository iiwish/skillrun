...
SkillRun v0.5.0 是当前 release candidate 版本。最近完成的 mainline patch release 是 v0.4.3；v0.5.0 定义 language-agnostic Adapter Protocol，并用 Level 0 command adapter 证明这条边界。v0.5.2 integration line 增加 Consumer JSON Surface；v0.5.3 增加本地 capsule registry 与 switchboard state，服务后续 Router/Desktop。

- 当前实现：v0.2 MCP stdio 行为、v0.3 JS Action Alpha、v0.4 Portable Consumer Checks、v0.4.1 WeCom Team Notice、v0.4.2 官方参考胶囊、v0.4.3 CI/runtime error stabilization、v0.5 Adapter Protocol 与 Level 0 command adapter runtime、v0.5.2 Consumer JSON Surface，以及 v0.5.3 本地 registry/switchboard state。
