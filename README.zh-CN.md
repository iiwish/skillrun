...
SkillRun v0.5.5 是当前 release binary/crate 版本。v0.5.0 定义 language-agnostic Adapter Protocol，并用 Level 0 command adapter 证明这条边界。v0.5.2 integration line 增加 Consumer JSON Surface；v0.5.3 增加本地 capsule registry 与 switchboard state；v0.5.4 稳定 Core contract；v0.5.5 在独立 Desktop 项目消费前硬化 Manifest-driven Consumer Mode。

- 当前实现：v0.2 MCP stdio 行为、v0.3 JS Action Alpha、v0.4 Portable Consumer Checks、v0.4.1 WeCom Team Notice、v0.4.2 官方参考胶囊、v0.4.3 CI/runtime error stabilization、v0.5 Adapter Protocol 与 Level 0 command adapter runtime、v0.5.2 Consumer JSON Surface、v0.5.3 本地 registry/switchboard state、v0.5.4 Core stabilization，以及 v0.5.5 Manifest-driven Consumer Mode hardening。
- 当前可用：`skillrun --help`、`skillrun --version`、`skillrun init <name> --python`、`skillrun init <name> --py`、`skillrun init <name> --js`、`skillrun manifest --cwd <capsule>`、`skillrun inspect --cwd <capsule>`、`skillrun inspect --json --cwd <capsule>`、`skillrun check --cwd <capsule>`、`skillrun check --json --cwd <capsule>`、`skillrun doctor --cwd <capsule>`、`skillrun doctor --json --cwd <capsule>`、`skillrun registry add/list/inspect/remove`、`skillrun switchboard list/enable/disable`、`skillrun consumer inventory --json`、`skillrun consumer exposure --json`、`skillrun consumer runs list --json`、`skillrun consumer mount plan --client <id> --json`、`skillrun test --cwd <capsule>`、`skillrun run --cwd <capsule> --input <file>`、`skillrun serve --mcp --cwd <capsule>`、`skillrun serve --mcp --cwd <capsule> --dry-run`、`skillrun pack --cwd <capsule>`、structured error envelopes、`DependencyError`、artifact validation、declared env injection、stale Manifest guards、instruction-only guards、Manifest-derived MCP tools/resources、`.skr` package generation，以及 skeleton/init/manifest/inspect/check/doctor/registry/switchboard/runtime/error/artifact/permission/consumer-guard/MCP/pack 路径的 release tests。
...
- `Cargo.toml` 与 `skillrun --version` 标识 binary/crate version。
- `v0.4.3` 这类 Git tag 标识公开 release 边界。
- v0.5.2、v0.5.3、v0.5.4、v0.5.5 这类 milestone 名称描述 integration scope，不自动等于已发布 tag。
- Manifest `manifest_version` 标识 Manifest IR schema。
- IPC / Adapter `protocol_version` 标识 Core 到 adapter process 的文件协议。

当前本地 binary 输出 `skillrun 0.5.5`；当前生成的 Manifest IR 与 IPC protocol version 仍是 `0.1.0`。v0.5.5 硬化 Manifest-driven runtime 行为和 Consumer Mode contract，但不改变这些协议版本。
...
v0.5.2 中，`inspect`、`check` 和 `doctor` 同时提供 `--json`，给 Desktop、Router 和自动化消费。`test` 和 `run` 不变，因为它们已经输出标准 output/error envelope JSON。

v0.5.3 中，`registry` 记录本地 capsule inventory，`switchboard` 记录未来暴露意图。它不导入 `.skr`、不暴露 MCP tools、不挂载客户端、不安装依赖、不认证 trust，也不提供 sandbox。

v0.5.4 中，Core hardening 让 command readiness 不执行任意 executable、让 Manifest schema 成为 runtime contract、让坏 registry entry 按单条降级，并冻结 Desktop-facing JSON fixture。Desktop 应作为独立项目消费这些稳定 Core surface；它不应重新定义 Manifest schema、直接执行 action，或把 MCP text content 当作审计数据源。

v0.5.5 中，Consumer Mode 在执行、MCP 暴露和 `.skr` 分发前共享 Manifest 静态合同。非法 runtime 字段或 schema contract 会在创建 run record、MCP tool contract 或 package archive 前 fail-closed。

v0.5.6 release-polish line 增加 `skillrun consumer inventory --json`、`skillrun consumer exposure --json`、registry-scoped `skillrun consumer runs list --json` summary，以及 plan-only `skillrun consumer mount plan --client <id> --json`，作为显式 headless consumer control-plane surface。它们复用 registry/runtime evidence 语义，不引入 Desktop、daemon、sandbox、signed package trust、marketplace 或真实 MCP client config mutation。
...
- `registry` 是本地 inventory，不是 marketplace、trust registry 或 install source。
- `switchboard enabled=true` 是未来 Router 的 exposure intent，不是 trust、sandbox、dependency installation 或 MCP client mounting。
- Desktop 是未来独立项目。它应该消费 CLI JSON contract、registry/switchboard state、run record 和后续 Core API；不应该绕过 `check`、把 `.skillrun/` 内部目录当作稳定 API、重新定义 Manifest 语义，或从 MCP text content 反解析审计证据。
...
- v0.5.5 release handoff 是显式发布动作：merge、tag creation、remote push 和 GitHub Release publication 属于本次发布；package publication 仍然是单独显式决策。
