...
SkillRun v0.4.3 是当前 patch release candidate 版本。最新 public release 是 v0.4.0；v0.4.3 稳定 v0.4.2 release line，修复 Linux CI 失败，并统一缺失 metadata runtime 时的错误文案。

- 当前实现：v0.2 MCP stdio 行为、v0.3 JS Action Alpha、v0.4 Portable Consumer Checks、v0.4.1 WeCom Team Notice 示例、v0.4.2 官方参考胶囊，以及 v0.4.3 CI/runtime 错误稳定化。
...
- `v0.4.0` tag 和 public release 已经发布。v0.4.3 tag creation、remote push 和 package publication 仍然是合并获批后的单独显式决策。
...
当前可运行示例仍刻意保持收敛：`refund` 证明安全与审计边界，`wecom_team_notice` 证明更贴近日常工作的本地通知场景，但不把 SkillRun 变成企业微信 adapter 或 API wrapper。v0.4.2 参考胶囊展示可复用 preflight 模式，但不声称 OS sandbox、registry trust 或通用 shell。
