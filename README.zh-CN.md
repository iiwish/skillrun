...
SkillRun v0.5.4 是当前 release binary/crate 版本。v0.5.0 定义 language-agnostic Adapter Protocol，并用 Level 0 command adapter 证明这条边界。v0.5.2 integration line 增加 Consumer JSON Surface；v0.5.3 增加本地 capsule registry 与 switchboard state；v0.5.4 在独立 Desktop 项目消费 Core 前硬化运行时和 JSON contract。

- 当前实现：v0.2 MCP stdio 行为、v0.3 JS Action Alpha、v0.4 Portable Consumer Checks、v0.4.1 WeCom Team Notice、v0.4.2 官方参考胶囊、v0.4.3 CI/runtime error stabilization、v0.5 Adapter Protocol 与 Level 0 command adapter runtime、v0.5.2 Consumer JSON Surface、v0.5.3 本地 registry/switchboard state，以及 v0.5.4 Core contract hardening。
...
- v0.5.2、v0.5.3、v0.5.4 这类 milestone 名称描述 integration scope，不自动等于已发布 tag。
- Manifest `manifest_version` 标识 Manifest IR schema。
- IPC / Adapter `protocol_version` 标识 Core 到 adapter process 的文件协议。

当前本地 binary 输出 `skillrun 0.5.4`；当前生成的 Manifest IR 与 IPC protocol version 仍是 `0.1.0`。v0.5.4 硬化 Core 行为和 JSON contract，但不改变这些协议版本。
...
- v0.5.4 release handoff 是显式发布动作：merge、tag creation 和 remote push 属于本次发布；package publication 仍然是单独显式决策。
