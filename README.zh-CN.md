...
- 当前实现：已推进到 `T007` artifact containment 和 declared env handling。
- 当前可用：`skillrun --help`、`skillrun --version`、`skillrun init <name> --python`、`skillrun manifest --cwd <capsule>`、`skillrun inspect --cwd <capsule>`、`skillrun test --cwd <capsule>`、`skillrun run --cwd <capsule> --input <file>`、structured error envelopes、artifact validation、declared env injection，以及 CLI/init/manifest/inspect/runtime/error/artifact/permission 路径的 contract tests。
